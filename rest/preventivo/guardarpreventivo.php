...
function guardapreventivo()
	...
	function notificarCEstado($incidente,$notificar,$accion,$estadoold,$estadonew,$usuario,$user_id,$user_nivel){
		...
		$query  = " SELECT a.id, a.titulo, a.descripcion, b.nombre AS proyecto, c.codigo AS codigounidad, c.nombre AS ambiente,
					...
					IFNULL(j.correo, a.solicitante) AS correosolicitante, a.idclientes, a.idproyectos
					FROM incidentes a
	...
	function notificarCAsignadoa($incidente,$notificar,$accion,$asignadoaInc,$asignadoa){
		...
		enviarMensajeIncidente($asunto,$mensaje,$correo,'','');
                                         ...
                                         //,$correos
	function enviarMensajeIncidente($asunto,$mensaje,$adjuntos,$tipo) {
		global $mysqli, $mail;
		$correo[] ='christopher.carnevale.p@gmail.com'; //array_unique($correos);
		$cuerpo = "";
	    ...
	    //acitvar $mail->addAddress("lismary.18@gmail.com");
