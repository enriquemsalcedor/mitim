<?php
		...
		while($row = $result->fetch_assoc()){
			...
			$acciones = '<td>
							<div class="dropdown ml-auto text-right">
								...
								$acciones .= '<a class="dropdown-item text-'.$color.' boton-evidencias"  data-id='.$row['id'].' "><i class="fas fa-camera mr-2"></i>Adjuntos</a>'; 
			...
			$acciones = '<td>
							<div class="dropdown ml-auto text-right">
								...
								<div class="dropdown-menu dropdown-menu-right">
									...
									<a class="dropdown-item text-danger boton-eliminar-comentarios" data-id="'.$row['id'].'"><i class="fas fa-trash mr-2"></i>Eliminar</a><a class="dropdown-item text-'.$color.' boton-adjuntos-comentarios"  data-id='.$row['id'].' "><i class="fas fa-camera mr-2"></i>Adjuntos</a>'; 
	...
	function updatecomentario(){
		...
		$query 	= "	UPDATE compromisos SET estado = '".$estado."', resolucion = '".$resolucion."' WHERE id = '".$id."' ";
		$result = $mysqli->query($query);	
		//debug($query);
		if($result == true){		    
	...
	function notificarComentarios($incidente,$comentario,$visibilidad){
		...
		$mensaje  = "<div style='padding: 30px;font-family: arial,sans-serif;'>
					...
					<p style='width:100%;'><br><a href='http://toolkit.maxialatam.com/soporte/postventas.php?id=$incidente' target='_blank' style='background-color: #008fc9;color: #FFFFFF;padding: 10px 20px;border-radius: 4px;text-decoration: none;margin-left: 30px;'>Añadir un compromiso</a></p>
	...
	function notificarCEstado($incidente,$notificar,$accion,$estadoold,$estadonew){
		...
		$query  = " SELECT a.id, a.titulo, a.descripcion, b.nombre AS proyecto, c.codigo AS codigounidad, c.idambientes AS idambientes,
					a.serie, d.marca, d.modelo, e.nombre AS estado, f.id AS idcategorias, f.nombre AS categoria, g.nombre AS subcategoria,
					h.prioridad, a.origen, IFNULL(i.nombre, a.creadopor) AS creadopor, IFNULL(j.nombre, a.solicitante) AS solicitante, a.asignadoa,
					...
					a.horastrabajadas, a.comentariovisto,					
		...
		$mensaje .= "<p style='width:100%;'>
						<a href='http://toolkit.maxialatam.com/soporte/incidentes.php?id=".$incidente."' target='_blank' style='background-color: #008fc9;color: #FFFFFF;padding: 10px 20px;border-radius: 4px;text-decoration: none;margin-left: 30px;'>Ver Visita</a></p>
						<br><br>
