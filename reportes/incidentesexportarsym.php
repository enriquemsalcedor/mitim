<?php
	...
	->setCellValue('AA4', 'Fecha de primer comentario')
	->setCellValue('AB4', 'Fecha de asignación')
	->setCellValue('AC4', 'Fecha de en proceso')
	->setCellValue('AD4', 'Fecha de resolución')
	/*
	...
	*/	
	->setCellValue('AE4', 'Tiempo de servicio')
	->setCellValue('AF4', 'Horas Trabajadas')
	->setCellValue('AG4', 'Periodo')
	->setCellValue('AH4', 'Fuera de Servicio')
	->setCellValue('AI4', 'Fuera de Servicio desde')
	->setCellValue('AJ4', 'Fuera de Servicio hasta')
	->setCellValue('AK4', 'Dias Fuera de Servicio')
	->setCellValue('AL4', 'Atención');	
	...
	$query  = " SELECT a.id, a.titulo, a.descripcion, b.nombre AS proyecto, e.nombre AS estado, m.nombre AS equipo, m.serie, m.activo, 
				...
				(SELECT CONCAT(ie.fechadesde, ' ', ie.horadesde) FROM incidentesestados ie WHERE a.id = ie.idincidentes AND ie.estadonuevo = 13 ORDER BY ie.id DESC LIMIT 1) AS estadoasignado,
				(SELECT CONCAT(iep.fechadesde, ' ', iep.horadesde) FROM incidentesestados iep WHERE a.id = iep.idincidentes AND iep.estadonuevo = 33 ORDER BY iep.id DESC LIMIT 1) AS estadoenproceso,
	...
	while($row = $result->fetch_assoc()){
		...
		->setCellValue('Z'.$i, $xfechacreacion.' '.$row['horacreacion'])
		->setCellValue('AA'.$i, $fprimercomentario)
		->setCellValue('AB'.$i, $festadoasignado)
		->setCellValue('AC'.$i, $festadoenproceso)
		->setCellValue('AD'.$i, $xfecharesolucion.' '.$row['horaresolucion'])
		//->setCellValue('AF'.$i, $dif)
		->setCellValue('AE'.$i, $totalFinal)
		->setCellValue('AF'.$i, $row['horastrabajadas'])
		->setCellValue('AG'.$i, $row['periodo'])
		->setCellValue('AH'.$i, $fueraservicio)
		->setCellValue('AI'.$i, $row['fechadesdefueraservicio'])
		->setCellValue('AJ'.$i, $row['fechafinfueraservicio'])
		->setCellValue('AK'.$i, $row['diasfueraservicio'])
		->setCellValue('AL'.$i, $atencion);		
