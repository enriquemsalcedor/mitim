<?php
	...
	$query  = " SELECT a.id, a.titulo, a.descripcion, b.nombre AS proyecto, e.nombre AS estado, 
				m.nombre AS equipo, m.serie, m.activo, mar.nombre as marca, r.nombre as modelo, ti.nombre AS modalidad, m.estado as estadoequipo, 
				...
				a.horastrabajadas, cu.periodo, p.nombre as cliente, a.idestados, a.idprioridades, a.reporteservicio  
				FROM incidentes a
	...
	if($data != ''){
		...
		if(!empty($data->modalidadf)){
			...
			if($modalidadf != '[""]'){
				$where .= " AND ti.nombre IN ($modalidadf)";
			}
		}
	...
	if($bhorac != ''){
		$where .= " AND a.horacreacion like '%".$bhorac."%' ";
	}
	if($bfechar != ''){
		$where .= " AND a.fechareal like '%".$bfechar."%' ";
	}
	if($bempresa != ''){
	...
	if($bmodalidad != ''){
		$where .= " AND ti.nombre like '%".$bmodalidad."%' ";
	}		
	if($bserie != ''){
	...
	if($bcierre != ''){
		$where .= " AND a.fecharesolucion LIKE '%".$bcierre."%' ";
		//$where .= " AND a.fechacierre LIKE '%".$bcierre."%' ";
	}
	$query  .= " $where ORDER BY a.id DESC ";
	//$query  .= " LIMIT 10 ";
	//debugL($query,"EXPORTARPREVENTIVOS");
	...
	while($row = $result->fetch_assoc()){
		...
		$arr [] = $xfechacreacion;
		$arr [] = $row['horacreacion'];
		$arr [] = $xfecharesolucion;
		$arr [] = $row['horaresolucion'];
		$arr [] = $xfechacierre;
		$arr [] = $row['horacierre'];
		$arr [] = $xfechavencimiento;
		$arr [] = $row['horavencimiento'];
		$arr [] = $xfechareal;
		$arr [] = $row['horareal'];
		$arr [] = $totalFinal;
		$arr [] = $row['horastrabajadas'];
		$arr [] = $row['periodo']; 
		$arr [] = $row['reporteservicio'];
		$arrayData [] = $arr; 
