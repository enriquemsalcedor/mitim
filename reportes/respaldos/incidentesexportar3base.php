<?php
	...
	*/	
	...
	function diferencia($fechadesde,$horadesde,$fechahasta,$horahasta){
		...
		debugL("VALORES:".$horas.":".$minutos.":".$segundos);
	...
	while($row = $result->fetch_assoc()){
		...
		$total14 = $sumaABG14; 
		debugL("total14:".$total14); 
		
		//----------------------------ESTADO: 42 - Reporte Pendiente --------------------------------------//
		...
		if($existe14==1){
			if($total14 > $totalGeneral){
				$totalFinal = $totalGeneral;
				debugL('TF1');
			}else{
				$totalFinal = resta($totalGeneral,$total14);
				debugL('TF2');
			}
	...
	function obtenerHorasEstados($horariocierre,$horarioinicio){
