...
$("#icono-filtrosmasivos,#icono-limpiar,#icono-refrescar").css("display","none");
	...
	iddepartamento =getUrl!==false?getUrl:0;  

	tabladepartamentoscp = $("#tabladepartamentoscp").DataTable({
		responsive: false,
		destroy: true,
		ordering: false,
		searching: false,
		ajax: {
			url:`controller/departamentosback.php?oper=cargardepartamentosclientes&iddepartamento=${iddepartamento}`,
		},
		columns	: [
			{ 	"data": "id" },			//0
			{ 	"data": "acciones" },	//1 
	...
	const guardar = ()=>{
		...
		if(vasociar(idcliente,idproyecto)== 1&& respuesta == 0){
			$.ajax({
				type: 'post', 
				url: `controller/departamentosback.php?oper=asociardepartamentosclientes`,
				data: {   
					'id'   	 	: id, 
					'idcliente' : idcliente,
				...
				success: function (response) {	
					if(response == 1){ 
						vaciarCampos();
						swal({		
									title: 'Departamento asociado satisfactoriamente',	
									text: "¿Desea asociar otra departamento?",
									type: "success",
								...
								if (isConfirm.value === true) {
									tabladepartamentoscp.ajax.reload(null, false);
	...
	if(getUrl!==false){
	    jQuery.ajax({
           url: "controller/departamentosback.php?oper=getdepartamentos&iddepartamentos="+iddepartamento,
           dataType: "json",
           success: function(item) {
				let tipo = item.tipo;
				tipo === 'departamento' ? texto = 'Departamento' : texto = 'Grupo'
                $("#nombredepartamento").text(`${tipo}: ${item.nombre}`);   
                $('.tipo').html(item.tipo);   
           }, 
        }); 
	}
	...
	$(document).on('click','.boton-eliminar', function(e){
		...
		var iddepartamento = $(this).attr("data-iddepartamento"); 
		
		eliminar(id,idcliente,idproyecto,iddepartamento);
		
	});
	
	//Quitar asociación
	const eliminar = (id,idcliente,idproyecto,iddepartamento)=>{
		 $.get( "controller/departamentosback.php?oper=hayRelacionPc", 
		{  
			id 		       : id,
			idcliente      : idcliente,
			idproyecto     : idproyecto,
			...
			if(result == 1){
				notification('El departamento tiene registros asociados, no se puede eliminar.','Advertencia','warning');
			}else{
				swal({
					title: "Confirmar",
					text: "¿Esta seguro de eliminar la asociación al departamento ?",
					type: "warning",
					showCancelButton: true,
					cancelButtonColor: 'red',
					confirmButtonColor: '#09b354',
					...
					function(isConfirm){
						if (isConfirm.value === true) {
							$.get( "controller/departamentosback.php?oper=eliminardepartamentosclientes", 
							{  
								id : id
