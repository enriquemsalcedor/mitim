...
function fsubcategoria(idcategoria){	
	//OBTENER COMBO DE SUBCATEGORIAS
	$("#gs_idsubcategoria").html('');
	$.get( "controller/combosback.php?oper=subcategorias&idcategoria="+idcategoria, { form:"true" }, function(result){ 
		$("#gs_idsubcategoria").html(result);
	});
}
...
//$(document).ready(function() {
	...
	$.get( "controller/combosback.php?oper=proyectos", { onlydata:"true" }, function(result){ 
		$("#proyecto, #proyectof").append(result);
		$("#proyecto, #proyectof").select2();
	});
	//SITIOS
	$.get( "controller/combosback.php?oper=unidades", { onlydata:"true" }, function(result){ 
		$("#unidadejecutora, #unidadejecutoraf").append(result);
		$('#unidadejecutora, #unidadejecutoraf').select2();
	});
	//MODALIDADES
	...
	//PROYECTOS
	$("#proyecto").change(function(e,data){
		console.log(data);
		var idproyecto = $("#proyecto option:selected").val();	
        $.get( "controller/combosback.php?oper=categorias&tipo=incidente&idproyecto="+idproyecto, { onlydata:"true" }, function(result){ 
			$("#categoria, #categoriaf, #filtrocat").empty();
			$("#categoria, #categoriaf, #filtrocat").append(result);
			if (data !== undefined) {
				console.log(data.pcategoria);
				$("#categoria, #categoriaf, #filtrocat").val(data.pcategoria);
			}
			$("#categoria, #categoriaf, #filtrocat").select2();
		});
	});
	//CATEGORIAS	
	$.get( "controller/combosback.php?oper=categorias&tipo=incidente", { onlydata:"true" }, function(result){ 
		$("#categoria, #categoriaf, #filtrocat").empty();
		$("#categoria, #categoriaf, #filtrocat").append(result);
		$("#categoria, #categoriaf, #filtrocat").select2();
	});	
	...
	$.get( "controller/combosback.php?oper=prioridades", { onlydata:"true" }, function(result){ 
		$("#prioridad, #prioridadf").append(result);
		$("#prioridadf").select2();
	});
	//SOLICITANTE
	$.get( "controller/combosback.php?oper=usuarios", { onlydata:"true" /*, nivel:"14"*/}, function(result){ 
		$("#solicitante, #solicitantef").append(result);
		$('#solicitante, #solicitantef').select2();
	});
	//ASIGNADO A
	$.get( "controller/combosback.php?oper=usuariosGrupos", { onlydata:"true", nivel:"2,3" }, function(result){ 
		$("#asignadoa, #asignadoaf").empty();
		$("#asignadoa, #asignadoaf").append(result);
		$('#asignadoa, #asignadoaf').select2();		
	});
	$("#asignadoa").change(function(){
		var asignadoa = $("#asignadoa option:selected").val();
	});
	//NOTIFICAR A
	...
	tablaincidentes = $("#tablaincidentes").DataTable({
		...
		colReorder: true,
		ajax		: {
			url	: "controller/actividadesback.php?oper=incidentes&f=" + paramf
		},
		columns	: [
			{ 	"data": "check"},
			{ 	"data": "acciones"},
		...
		initComplete: function () {
			...
			this.api().columns().every( function () {
                var column = this;
				var titles = ["Categoría","Sub Categoría","Estado","Proyecto","Asignado a","Sitio","Prioridad"];
				if (column[0]==4 || column[0]==5 || column[0]==6 || column[0]==9 || column[0]==10 || column[0]==11 || column[0]==12) {
					var select = $('<select><option value="" disabled selected>' + titles[t] + '</option></select>')
	...
	$('#tablaincidentes tbody').on('click', 'td:not(:first-child)', function () {
		$(this).closest('tr').toggleClass('selected');
	});  
	
	$('#chkSelectAll').click(function(){
		if($("#chkSelectAll").is(':checked'))
			tablaincidentes.rows('current');
		else
			tablaincidentes.rows('none');
	});
	...
	function eliminarincidente(id,nombre){
		...
		swal({
			...
			function(isConfirm){
				if (isConfirm){
					$.get( "controller/actividadesback.php?oper=eliminarincidentes", 
					{ 
						onlydata : "true",
