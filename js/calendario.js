var grid_coment_form 	= "#grid-coment-form";
var pager_coment_form 	= "#pager-coment-form";
var incidenteselect = '';
$('#cmbFiltroResponsable').select2();
$('#cmbFiltroMarca').select2();
var urlabrir = '';
...
$(document).ready(function() {
    ...
    $('#calendar').fullCalendar({
        ...
        eventClick:  function(event, jsEvent, view) {
			incidenteselect = event.id;
			abrirdialogIncidenteEditar(event.id);
        }
    })
...
//VALIDAR CAMPOS 
$(".form-control").bind("keydown blur",function(e){campos(e,this);});
function campos(e,t) { 
...
$('#fechareal_editar, #fecharesolucion_editar').bootstrapMaterialDatePicker({weekStart:0, format:'YYYY-MM-DD HH:mm:ss', switchOnClick:true });
...
function abrirdialogIncidenteEditar(id){
	...
	$.ajax({
		...
		success: function (respuesta) { 
			...
			if(respuesta == 12 || respuesta == 22 || respuesta == 35 || respuesta == 43){ 
				urlabrir = 'controller/preventivosback.php';
				$('.inpatencion').hide();
				$('.inpfechreal').show();
				$(".text_tipo").html('Preventivo');
				$("#tipo_registro").val('prev');
			}else{ 
				urlabrir = 'controller/incidentesback.php';
				$('.inpatencion').show();
				$('.inpfechreal').hide();
				$(".text_tipo").html('Incidente');
				$("#tipo_registro").val('inc');
			}
			
			var tipo_registro = $("#tipo_registro").val();
			if(tipo_registro == 'inc'){
				tipo = 'incidente'; 
			}else{
				tipo = 'preventivo'; 
			}
			...
			$('#idproyectos_editar').on('select2:select',function(){
				...
				$.get( "controller/combosback.php?oper=categorias", { tipo: tipo, idproyectos: idproyectos }, function(result){ 
			...
			success: function (response) {
				...
				$.map(response, function (item) {
					...
					$.when( $('#idproyectos_editar').triggerHandler('change') ).then(function( data, textStatus, jqXHR ) {
						$.get( "controller/combosback.php?oper=categorias", { tipo: tipo, idproyectos: item.idproyectos }, function(result){ 
							$("#categoria_editar").empty();
							$("#categoria_editar").append(result);
							$("#categoria_editar").select2({placeholder: ""});
					...
					$('#fechareal_editar').val(item.fechareal).trigger("change");						
				});
			},
		...
		$('#modaleditincidentes').modal('show');
		$('#modaleditincidentes .modal-lg').css('width','1000px');
		},
		error: function () {
...
function cerrarDialogIncidenteEditar() {
	...
	$("#modaleditincidentes").modal("hide");
}
...
/* var dialogAdj;
var optionsAdj = {
	url  : 'elFinder/php/connector.minimal.incidentes.php?idincidente=0',
	lang : 'es',
	...
	if ( valid ) {
		$.ajax({
			  ...
			  success: function (response) {
				...
				$('.titulo-modal').text('Adjuntos de comentarios');
				$('#modalEvidencias').modal('show');
				$('#modalEvidencias').css('z-index','1150');
...
function eliminarcomentario(id){
	var idincidente  = $("#incidente_editar").val();											 
	var idcomentario = id;
	swal({
		title: "Confirmar",
...
function limpiarCalendario() {
	//$("#frmFiltrosCalendario")[0].reset();
	$('#chkIncidentes')[0].checked = false;
	$('#chkPreventivas')[0].checked = true;
	$('#chkAbiertas')[0].checked = true;
	$('#chkCerradas')[0].checked = false;
	$('#cmbFiltroResponsable').val("").change();
	$('#cmbFiltroMarca').val("").change();		
	filtrarCalendario();
}

function filtrarCalendario() {
	/*arrResponsable = $('#cmbFiltroResponsable').select2("val");
	arrMarca = $('#cmbFiltroMarca').val();
	for (i=0; i < arrResponsable.length; i++) {
		arrResponsable[i] = "'" + arrResponsable[i] + "'";
	}
	for (i=0; i < arrMarca.length; i++) {
		arrMarca[i] = "'" + arrMarca[i] + "'";
	}*/
	var events = {
		url: 'controller/calendarioeventos.php',
		type: 'POST',
		data: {
			oper: 'eventos',
			chkt: $('#chkTodos').is(":checked"),
			chki: $('#chkIncidentes').is(":checked"),
			chkp: $('#chkPreventivas').is(":checked"),
			chka: $('#chkAbiertas').is(":checked"),
			chkc: $('#chkCerradas').is(":checked"),
			cmbr: $('#cmbFiltroResponsable').select2("val"),
			cmbm: $('#cmbFiltroMarca').select2("val")
		},
		error: function() {
			alert('Hubo un error al cargar los eventos!');
		}
