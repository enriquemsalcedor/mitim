...
function abrirdialogImportar(){
	...
	$("#resultado").html('');
	//dialogImportarActivos.dialog( "open" );
...
$('#descargarplantilla').click(function(){
	...
	if(idclientes != "0" && idproyectos != "0" && idclientes != null && idproyectos != null){
		window.open('reportes/descargarplantillaactivos.php?idclientes='+idclientes+'&idproyectos='+idproyectos);
	}else{ 
		notification("¡Error!","Debe llenar los campos Clientes y Proyectos","error");  
	}   
});
...
$('#subir-archivo').click(function(){  
	...
	if(nombredocumento != ''){
		$.ajax({
			url : 'reportes/activosimportar.php',  
			type : 'POST',
			data : formData,
			dataType: 'html',
