...
0}}}),e.bDiv=document.createElement("div"),s&&"auto"===String(d.p.height).toLowerCase()&&(d.p.height="100%"),$(e.bDiv).append($('<div style="position:relative;"></div>').append("<div></div>").append(this)).addClass("ui-jqgrid-bdiv").css({width:e.width-Ga+"px"}).scroll(e.scrollGrid),$("table:first",e.bDiv).css({width:d.p.tblwidth+"px"}),$.support.tbody||2===$("tbody",this).length&&$("tbody:gt(0)",this).remove(),d.p.multikey&&($.jgrid.msie()?$(e.bDiv).on("selectstart",function(){return!1}):$(e.bDiv).on("mousedown",function(){return!1})),Ea&&$(e.bDiv).hide();var
...
d,e,f,g="";return this.each(function(){g=this.formatCol(b,c),e=this.p.id,d=this.p.subGridOptions.plusicon,f=a.jgrid.styleUI[this.p.styleUI||"jQueryUI"].common}),'<td role="gridcell" aria-describedby="'+e+'_subgrid" class="ui-sgcollapsed sgcollapsed" '+g+"><a style='cursor:pointer;' class='ui-sghref'><button type='button' rel='tooltip' class='ui-sghref btn btn-info' data-original-title='' title=''><i class='material-icons'>visibility</i><div class='ripple-container'></div></button></a></td>"},addSubGrid:function(b,c){return this.each(function(){var
d=this;if(d.grid){var
