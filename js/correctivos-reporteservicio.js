...
function generarReporte(){
	...
	if(tecnico == '' || tecnico == undefined){ 
		demo.showSwal('error-message','ERROR','Debe llenar el campo Nombre de Técnico');
	}else if(tiposervicio == '' || tiposervicio == undefined || tiposervicio == 'sinasignar'){ 
		demo.showSwal('error-message','ERROR','Debe llenar el campo Tipo de Servicio');
	}else if(estadoactivo == '' || estadoactivo == undefined || estadoactivo == 'sinasignar'){ 
		demo.showSwal('error-message','ERROR','Debe llenar el campo Estado Final del activo');
	}else if(fallareportada == '' || fallareportada == undefined){ 
		demo.showSwal('error-message','ERROR','Debe llenar el campo Falla o error reportado');
	}else if(trabajorealizado == '' || trabajorealizado == undefined){ 
		demo.showSwal('error-message','ERROR','Debe llenar el campo Trabajo realizado');
	}else if(observaciones == '' || observaciones == undefined){ 
		demo.showSwal('error-message','ERROR','Debe llenar el campo Observaciones');
	}else if(fechafirmatecnico == '' || fechafirmatecnico == undefined){ 
		demo.showSwal('error-message','ERROR','Debe llenar el campo Fecha de firma del reporte');
	}else if(nombrecliente1 == '' || nombrecliente1 == undefined){ 
		demo.showSwal('error-message','ERROR','Debe llenar el campo Nombre 1 del Cliente');
	}else if(idreporte == "" && firmatecnico == firmalimpia){
		demo.showSwal('error-message','ERROR','Debe colocar la Firma del Técnico');
	}else if(idreporte == "" && firmacliente1 == firmalimpia){
		demo.showSwal('error-message','ERROR','Debe colocar la Firma del Cliente 1');
		...
		demo.showSwal('error-message','ERROR','Debe llenar el campo Fecha de firma del Cliente');
	}else if(idreporte == ""){
		demo.showSwal('error-message','ERROR','Debe guardar los datos del Formulario');
	}else{
		//Validar datos en la bd
		 $.ajax({
			url: 'controller/incidentes-reporteservicioback.php',
			type : 'POST',
			dataType: 'json',
			data: { 
					...
					$.ajax({
						...
						success: function (response) { 
							if(response == 1){ 
								...
								demo.showSwal('error-message','ERROR!','Debe agregar la(s) fecha(s) de atención.');
							}
						} 
					});
				}else if(response == 0){
					demo.showSwal('error-message','ERROR','Ocurrió un error al generar el reporte');
				}else{
					demo.showSwal('error-message','ERROR',''+response.msg+'');
				}
			} 
		});  
	} 
...
$('#tiposervicio').select2({placeholder: ""}); 
$('#estadoactivo').select2({placeholder: ""}); 
...
function abrirFechas(id,tipo,idreporte){
	if(id!=undefined){
		if(tipo=='temp'){
			...
			var url = "controller/incidentes-reporteservicioback.php?oper="+oper+"&idincidente="+id;
		}else{
			var oper = 'listarFechasDef';
			var url = "controller/incidentes-reporteservicioback.php?oper="+oper+"&idincidente="+id+"&idreporte="+idreporte;
		}
...
function abrirRepotServcGn(id){
    if(id!=undefined)
	{
		...
		let url = "controller/incidentes-reporteservicioback.php?oper="+oper+"&idincidente="+id;
	    console.log("ejecuto ")
			...
			success: function (response) { 
				if(response==true){
					$.ajax({
						...
						success: function (response) { 
							if(response==1){
								demo.showSwal('success-message','Buen trabajo','Registro actualizado satisfactoriamente');
...
function agregarFechas(){
	...
	if(fechaatencion == '' || fechaatencion == undefined){ 
		demo.showSwal('error-message','ERROR','Debe llenar el campo Fecha de atención');
	}else if(tiempoviaje == '' || tiempoviaje == undefined){ 
		demo.showSwal('error-message','ERROR','Debe llenar el campo Tiempo de viaje');
	}else if(horainicio == '' || horainicio == undefined ){ 
		demo.showSwal('error-message','ERROR','Debe llenar el campo Hora Inicio');
	}else if(horafin == '' || horafin == undefined){ 
		demo.showSwal('error-message','ERROR','Debe llenar el campo Hora final');
	}else{
		$.ajax({
			type: 'post',
			dataType: "json",
...
function agregarRepuestos(){
	...
	if(codigo == '' || codigo == undefined){ 
		demo.showSwal('error-message','ERROR','Debe llenar el campo Código');
	}else if(cantidad == '' || cantidad == undefined){ 
		demo.showSwal('error-message','ERROR','Debe llenar el campo Cantidad');
	}else if(descripcion == '' || descripcion == undefined){ 
		demo.showSwal('error-message','ERROR','Debe llenar el campo Descripción');
	}else{
		$.ajax({
			type: 'post',
			dataType: "json",
 ...
 $('#tablarepuestostemp').on( 'draw.dt', function () {
	...
	function eliminarFechas(id,nombre,tipo){
		...
		swal({
			...
			function(isConfirm){
				if (isConfirm){
					$.get( "controller/incidentes-reporteservicioback.php?oper="+oper, 
					{ 
						onlydata : "true",
						id : idincidente
					}, function(result){
