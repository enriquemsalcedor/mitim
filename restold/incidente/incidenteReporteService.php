<?php
	...
	function guardarReporte(){
		...
		//(!empty($_REQUEST['usuario']) ? $_REQUEST['usuario'] : 0)
		...
		if($usuario != ""){
			...
			if($result == true){
				...
				$generarcodigo = " UPDATE reporteservicios SET codigo = 'RS-".$idincidente."' WHERE id = ".$idreporte." ";
				...
				//BITACORA
				bitacora($usuario, "Incidentes", "Se ha guardado el reporte de servicio del correctivo  #".$idincidente, $idincidente, $query);
				echo 1;
			}else{
				echo 0;
			}
	...
	function editarReporte(){
		...
		if($result == true){
			...
			//BITACORA
			bitacora($usuario, "Incidentes", "Se ha editado el reporte de servicio del correctivo  #".$idincidente, $idincidente, $query);
			echo true;
		}else{
			echo false;
		} 
	...
	function consultarCorrectivo(){
		global $mysqli;
		$id = (!empty($_REQUEST['idIncidente']) ? $_REQUEST['idIncidente'] : 0);
		$resultado 	 = array();
...
/*-------------------------------------------------------------------------------------------------------------------------*/
	...
	function listarFechasTemp(){
		...
		$idincidente   		 = (!empty($_REQUEST['idincidente']) ? $_REQUEST['idincidente'] : 0);
	    $usuario= (!empty($_REQUEST["usuario"]) ? $_REQUEST["usuario"] : 0);	 
	...
	function nuevoReporte(){
		...
		if($usuario != ""){
			...
			if($result == true){
				...
				//BITACORA
		        bitacora($usuario, "Incidentes", "Se ha guardado el reporte de servicio del preventivo  #".$idincidente, $idincidente, $query);
				echo 1;
			}else{
				echo 0;
			}
