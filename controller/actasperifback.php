<?php
	...
	$modalidad = utf8_encode('Periférico');
	...
	*/
	$query	= " SELECT c.fechafin as fechacertificar, b.nombre AS unidad, b.codigoaceptacion, c.numero, c.periodo 
				...
				WHERE a.idcategorias = 22 AND c.periodo = '".$periodo."'  AND d.modalidad = '".$modalidad."'
				AND a.idambientes = b.id  
				LIMIT 1";
	//echo $query;
	//debug($query);
	...
	*/
	...
	$query = "	SELECT a.id as incidente, a.idestados, a.reporteservicio, a.fechacreacion, a.fecharesolucion, a.observaciones,
				c.serie, c.nombre AS equipo, mar.nombre AS marca, mo.nombre AS modelo
				FROM incidentes a
				LEFT JOIN ambientes b ON a.idambientes = b.id
				...
				WHERE a.idcategorias = 22 
				AND (a.idestados = 16 OR a.idestados = 17) AND d.periodo = '".$periodo."' AND c.modalidad = '".$modalidad."' AND a.fecharesolucion is not NULL
	...
	$queryAnexos  = "SELECT a.id, a.reporteservicio 
					 ...
					 LEFT JOIN activos c ON a.idactivos = c.id
					 WHERE b.periodo = '".$periodo."' AND c.modalidad = '".$modalidad."'
					 AND (a.idestados = 16 OR a.idestados = 17) AND a.idcategorias = 22 ";
	//echo $queryAnexos;
