<?php
	...
	function incidentes()
	{
		...
		$proyecto = 13;
		$query  = " SELECT a.id, e.nombre AS estado, a.titulo, 
					b.nombre AS proyecto, c.unidad AS unidadejecutora,
					a.fechacreacion, a.horacreacion, 
					l.nombre as asignadoa, f.nombre AS categoria, g.nombre AS subcategoria, h.prioridad,
					a.fechacierre
					FROM actividades a
		...
		if ($proyecto!=0) {
			$query  .= "AND a.proyecto in ($proyecto) ";
			$queryCuenta  .= "AND a.proyecto in ($proyecto) ";
		}
		...
		if($nivel == 3) {
			if ($_SESSION['sitio']=='Implementacion') {
				...
				$query  .= "AND (l.usuario = '".$_SESSION['usuario']."' OR j.usuario = '".$_SESSION['usuario']."') ";
				$queryCuenta .= "AND (l.usuario = '".$_SESSION['usuario']."' OR j.usuario = '".$_SESSION['usuario']."') ";
			}
		} elseif($nivel == 4){
			if($_SESSION['sitio'] != ''){
				...
				$queryCuenta  .= "AND (j.usuario = '".$_SESSION['usuario']."') ";
			}			
		}
		...
		while($row = $result->fetch_assoc()){
			...
			$response['data'][] = array(				
				...
				'tieneEvidencias'	=> $tieneEvidencias
			);
		}
	...
	function comentarios(){
		...
		$query  = " SELECT a.id, a.idmodulo, a.comentario, a.fecha, b.nombre, a.visibilidad
					...
					WHERE modulo = 'Incidentes' AND idmodulo = 10632 ";
		if($nivel == 4){
