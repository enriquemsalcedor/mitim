...
function incidentes()
	{
		...
		if($data != ''){
			...
			if(!empty($data->idproveedoresf)){
				$idproveedoresf = json_encode($data->idproveedoresf);
				if($idproveedoresf != '[""]'){
					$idproveedoresf = str_replace('"','',$idproveedoresf);
					$where .= " AND l.idproveedor IN ($idproveedoresf)"; 
				}
			}
			if(!empty($data->fueraserviciof)){
				$fueraserviciof = json_encode($data->fueraserviciof); 
				$where .= " AND a.fueraservicio = 1 AND a.idestados != 16";
			}
			if(!empty($data->idetiquetasf)){
				$idetiquetasf = json_encode($data->idetiquetasf);
				$idetiquetasf = str_replace('"','',$idetiquetasf);
				$where .= " AND a.idetiquetas IN ($idetiquetasf) "; 
			}
			$vowels = array("[", "]");
		...
		if($searchGeneral!=""){
			$where.= " AND (
				...
				a.estadoant like '%".$searchGeneral."%' 
			) ";
		...
		while($row = $result->fetch_assoc()){
			...
			if($row['nomusuario'] != ""){
				if($row['nomusuario'] == $_SESSION['nombreUsuario']){
					$existeasignado = 1;
				} 
			}
			//Notificación en rojo, si lleva más de 24 horas asignados al asignado y al personal de soporte	
			if($row['fechacreacion'] != "" && $row['estado'] == "Asignado"){				
				if($existeasignado == 1 || $_SESSION['nivel'] == 1 || $_SESSION['usuario'] == "aporras" || $_SESSION['usuario'] == "frios" || $_SESSION['usuario'] == "icarvajal"){ 
					$horasasignado = diferencia($row['fechacreacion'],$row['horacreacion'],$fechahoy,$horahoy);
				}
			}
			//Notificación en rojo, si lleva más de 72 horas A la Espera del Cliente al asignado y al personal de soporte
			if($row['fechacreacion'] != "" && $row['estado'] == "A la Espera del Cliente"){				
				if($existeasignado == 1 || $_SESSION['nivel'] == 1 || $_SESSION['usuario'] == "aporras" || $_SESSION['usuario'] == "frios" || $_SESSION['usuario'] == "icarvajal"){ 
					$horasalaespera = diferencia($row['fechacreacion'],$row['horacreacion'],$fechahoy,$horahoy);
				}
			}
			//Notificación en rojo, si lleva más de 5 días en Reporte Pendiente al asignado y al personal de soporte
			if($row['fechacreacion'] != "" && $row['estado'] == "Reporte Pendiente"){				
				if($existeasignado == 1 || $_SESSION['nivel'] == 1 || $_SESSION['usuario'] == "aporras" || $_SESSION['usuario'] == "frios" || $_SESSION['usuario'] == "icarvajal"){ 
					$horasreportepen = diferencia($row['fechacreacion'],$row['horacreacion'],$fechahoy,$horahoy);
				}
			}
			$validacionusuario = 0;
			if($_SESSION['nivel'] == 1 || $_SESSION['usuario'] == "aporras" || $_SESSION['usuario'] == "frios" || $_SESSION['usuario'] == "icarvajal"){
				$validacionusuario = 1;
			} 
			 
			$row['idetiquetas'] != "" && $row['idetiquetas'] != "0"? $etiqueta = "<i class='fa fa-tags' style='color:".$row['color']."'></i> " : $etiqueta = "";
			if($_SESSION['nivel'] == 1 || $_SESSION['nivel'] == 2 || $_SESSION['nivel'] == 3 || $_SESSION['nivel'] == 5 || $_SESSION['nivel'] == 8){
				$etiqueta = $etiqueta;
			}else{
				$etiqueta = "";
			}
    	    ...
    	    $resultado[] = array(			
				...
				'estado' 			=> $row['estado'],
				//'titulo' 			=> $evid.' '.$titulo,		   
				'titulo' 			=> $etiqueta."<span data-toggle='tooltip' data-placement='right' data-original-title='".$row['titulott']."'>".$row['titulo']."</span>",
				...
				'horasasignado'		=> $horasasignado,
				'horasalaespera'	=> $horasalaespera,
				'horasreportepen'	=> $horasreportepen,
				'validacionusuario' => $validacionusuario,
				'etiqueta'			=> '<a class="badge badge-md ml-2 mr-2" style="width: 150px; color: #FFFFFF; cursor: pointer; background-color:'.$row["color"].'"><span data-toggle="tooltip" data-placement="right" data-original-title="'.$row['etiquetatt'].'">'.$row['etiqueta'].'</span></a>'
			);
		}
...
function clientes(){
			...
			$acciones = '<td>
							<div class="dropdown ml-auto text-center">
								...
								<div class="dropdown-menu dropdown-menu-center droptable"> 
										<a class="dropdown-item text-info" href="cliente.php?id='.$row['id'].'&type=edit"><i class="fas fa-pen mr-2"></i>Editar</a><a class="dropdown-item text-success boton-contactar" data-id="'.$row['id'].'"><i class="fa fa-phone mr-2"></i>Contactar</a>
									</div>
								</div>
							</td>';
	...
	function contactoclientes(){
		global $mysqli;
		
		$id 	= (!empty($_REQUEST['idclientes']) ? $_REQUEST['idclientes'] : 0);
		$query 	= "UPDATE clientes SET contactado = 1 WHERE id = '$id'";
		$result = $mysqli->query($query);	
		
		if($result == true){
		    
		    bitacora($_SESSION['usuario'], "Clientes", "El cliente #".$id." ha sido contactado", $id , $query);
