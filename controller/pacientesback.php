...
function mapa() {
	...
	$persona =  (!empty($_REQUEST['persona']) ? $_REQUEST['persona'] : "pacientes");
	
	if ($tipo!="")
		$consulta = " select sum($persona) as total ";
	else
		$consulta = " select $persona as total ";
	$consulta .= "
		from pacientes d
		where 1=1  
		";
	...
	$consulta = "
		...
		$persona as circle_options,
		$persona as html
		from unidades u
		inner join pacientes d on u.codigo = d.codigo 
		where d.codigo<>'CSS' ";
	...
	while($rec = $result->fetch_assoc()){
		...
		$objJson2["radius"] =  $rec['circle_options'] * 100 / $total * 3500 ;
...
function datos() {
	...
	$consulta = "
		   select ano, 'A' as modalidad, 
		   personas  as totali, pacientes  as totala
		   from pacientes 
		   where mes=0 and modalidad = 'Unidad' and codigo='CSS' and ano<=$agno and provincia = 'Provincia'
		   UNION 
		   select ano, modalidad, 
		   personas  as totali, pacientes  as totala
		   from pacientes 
		   where mes=0 and modalidad <> 'Unidad' and codigo='CSS' and ano<=$agno  and provincia = 'Provincia' ";								
	...
	while($row = $result->fetch_assoc()){
		...
		$response->rows[$i]['valori']=$row['totali'];
		$response->rows[$i]['valora']=$row['totala'];
		$i++;
	}        
	echo json_encode($response);
}
