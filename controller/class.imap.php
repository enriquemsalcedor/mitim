...
class Imap {
    ...
    private function processStructure($uid, $structure, $partIdentifier = null) {
        ...
        if ((isset($parameters['name']) || isset($parameters['filename'])) || (isset($structure->subtype) && strtolower($structure->subtype) == 'rfc822')
			...
			$encoding = mb_detect_encoding($messageBody, $encodings); 
			if ($encoding == 'ISO-8859-1')
				$messageBody = mb_convert_encoding($messageBody, "UTF-8", $encoding);
			else
			...
			//$messageBody = mb_convert_encoding($messageBody, 'UTF-8', 'US-ASCII');
