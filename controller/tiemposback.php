...
function mapa() {
	...
	$consulta = "
		select (sum(estudios*informe) / sum(estudios)) + sum(estudios*atencion) / sum(estudios) as total 
		from tiempos d
		where 1 = 1
		";
	if ($modalidad!="*") 
	...
	$consulta = "
		...
		(sum(estudios*informe) / sum(estudios)) + sum(estudios*atencion) / sum(estudios) as circle_options,
		(sum(estudios*informe) / sum(estudios)) + sum(estudios*atencion) / sum(estudios) as html
		from unidades u
		inner join tiempos d on u.codigo = d.codigo 
		where 1 = 1 ";
	...
	while($rec = $result->fetch_assoc()){
		...
		$objJson2["radius"] =  $rec['circle_options'] * 100 / $total * 100 ;
...
function datos() {
	...
	$consulta = "
			...
			sum(estudios*informe) / sum(estudios) as totali, 
			sum(estudios*atencion) / sum(estudios) as totala 
			FROM tiempos
			WHERE ano<=$agno ";								
	//debug($consulta);
