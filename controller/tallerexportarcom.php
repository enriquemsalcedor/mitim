...
//	->setCellValue('Z4', 'Comentario de Satisfacción')		
	->setCellValue('Q4', 'Fecha de creación')
	->setCellValue('R4', 'Hora de creación')
	->setCellValue('S4', 'Fecha de resolución')
	->setCellValue('T4', 'Hora de resolución')
	->setCellValue('U4', 'Fecha de cierre')
	->setCellValue('V4', 'Hora de cierre')
...
//	->setCellValue('AM4', 'Periodo')
	->setCellValue('W4', 'Comentarios');
	
	//LETRA
	$objPHPExcel->getActiveSheet()->getStyle('A4:AW4')->getFont()->setBold(true)->setSize(12)->setColor($fontColor);
	$objPHPExcel->getActiveSheet()->getStyle("A4:AW4")->applyFromArray($style);
	//FONDO
	$objPHPExcel->getActiveSheet()->getStyle('A4:AW4')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setRGB('63b9db');
	...
	$query  = " SELECT a.id, a.titulo, a.descripcion, b.nombre AS proyecto, e.nombre AS estado, 
					m.equipo, m.codequipo, a.serie as serie, m.activo, a.marca as marca, a.modelo as modelo, m.modalidad, m.estado as estadoequipo, 
					...
					cu.periodo, P.nombre as cliente, co.comentario 
					FROM taller a
	...
	//	->setCellValue('S'.$i, $row['satisfaccion'])
	//	->setCellValue('Z'.$i, $row['comentariosatisfaccion'])
		->setCellValue('Q'.$i, $xfechacreacion) //->setCellValue('W'.$i, implode('/',array_reverse(explode('-', $row['fechacreacion']))))
		->setCellValue('R'.$i, $row['horacreacion'])
		->setCellValue('S'.$i, $xfecharesolucion)// ->setCellValue('AA'.$i, implode('/',array_reverse(explode('-', $row['fecharesolucion']))))
		->setCellValue('T'.$i, $row['horaresolucion'])
		->setCellValue('U'.$i, $xfechacierre) //->setCellValue('AC'.$i, implode('/',array_reverse(explode('-', $row['fechacierre']))))
		->setCellValue('V'.$i, $row['horacierre'])
	...
	//	->setCellValue('AM'.$i, $row['periodo'])
		...
		//ESTILOS
		$objPHPExcel->getActiveSheet()->getStyle('A'.$i.':W'.$i)->getFont()->setSize(10);
		$objPHPExcel->getActiveSheet()->getStyle('A'.$i.':W'.$i)->getAlignment()->applyFromArray(
		...
		$objPHPExcel->getActiveSheet()->getStyle('V'.$i)->getNumberFormat()->setFormatCode('mm/dd/yyyy');
		$objPHPExcel->getActiveSheet()->getStyle('V'.$i)->getAlignment()->applyFromArray(
					array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_RIGHT));
		$objPHPExcel->getActiveSheet()->getStyle('R'.$i)->getNumberFormat()->setFormatCode('mm/dd/yyyy');
		$objPHPExcel->getActiveSheet()->getStyle('R'.$i)->getAlignment()->applyFromArray(
					array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_RIGHT));
		$objPHPExcel->getActiveSheet()->getStyle('T'.$i)->getNumberFormat()->setFormatCode('mm/dd/yyyy');
		$objPHPExcel->getActiveSheet()->getStyle('T'.$i)->getAlignment()->applyFromArray(
					array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_RIGHT));
	...
	$objPHPExcel->getActiveSheet()->setTitle('Incidentes - Taller');
